/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UserSessionWhereInput } from "./UserSessionWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { UserSessionOrderByInput } from "./UserSessionOrderByInput";

@ArgsType()
class UserSessionFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => UserSessionWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => UserSessionWhereInput, { nullable: true })
  @Type(() => UserSessionWhereInput)
  where?: UserSessionWhereInput;

  @ApiProperty({
    required: false,
    type: [UserSessionOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [UserSessionOrderByInput], { nullable: true })
  @Type(() => UserSessionOrderByInput)
  orderBy?: Array<UserSessionOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { UserSessionFindManyArgs as UserSessionFindManyArgs };
