/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ApiDocumentationWhereUniqueInput } from "./ApiDocumentationWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ApiDocumentationUpdateInput } from "./ApiDocumentationUpdateInput";

@ArgsType()
class UpdateApiDocumentationArgs {
  @ApiProperty({
    required: true,
    type: () => ApiDocumentationWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ApiDocumentationWhereUniqueInput)
  @Field(() => ApiDocumentationWhereUniqueInput, { nullable: false })
  where!: ApiDocumentationWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ApiDocumentationUpdateInput,
  })
  @ValidateNested()
  @Type(() => ApiDocumentationUpdateInput)
  @Field(() => ApiDocumentationUpdateInput, { nullable: false })
  data!: ApiDocumentationUpdateInput;
}

export { UpdateApiDocumentationArgs as UpdateApiDocumentationArgs };
