...
const service = {
  ...
  analyticsEvents: () => FIND_MANY_RESULT,
  analyticsEvent: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("AnalyticsEvent", () => {
  ...
  test("POST /analyticsEvents", async () => {
  ...
  test("GET /analyticsEvents", async () => {
    await request(app.getHttpServer())
      .get("/analyticsEvents")
  ...
  test("GET /analyticsEvents/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/analyticsEvents"}/${nonExistingId}`)
  ...
  test("GET /analyticsEvents/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/analyticsEvents"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /analyticsEvents existing resource", async () => {
