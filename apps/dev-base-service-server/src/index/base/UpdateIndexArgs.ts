/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IndexWhereUniqueInput } from "./IndexWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { IndexUpdateInput } from "./IndexUpdateInput";

@ArgsType()
class UpdateIndexArgs {
  @ApiProperty({
    required: true,
    type: () => IndexWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => IndexWhereUniqueInput)
  @Field(() => IndexWhereUniqueInput, { nullable: false })
  where!: IndexWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => IndexUpdateInput,
  })
  @ValidateNested()
  @Type(() => IndexUpdateInput)
  @Field(() => IndexUpdateInput, { nullable: false })
  data!: IndexUpdateInput;
}

export { UpdateIndexArgs as UpdateIndexArgs };
