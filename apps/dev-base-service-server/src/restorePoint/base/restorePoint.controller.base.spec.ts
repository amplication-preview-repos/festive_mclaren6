...
const service = {
  ...
  restorePoints: () => FIND_MANY_RESULT,
  restorePoint: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("RestorePoint", () => {
  ...
  test("GET /restorePoints/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/restorePoints"}/${nonExistingId}`)
  ...
  test("GET /restorePoints/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/restorePoints"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /restorePoints existing resource", async () => {
